...
function GenerateInvoicePage() {
  ...
  const recalculateItemWithRetention = (item, retentionAmount) => {
    const itemTotals = calculateItemTotals({
      quantity: item.quantity,
      unitPrice: item.unitPrice,
      saleType: item.saleType || 'GRAVADA',
      retention1: retentionAmount,
      fovial: usesFuelTaxes ? item.fovial || 0 : 0,
      cotrans: usesFuelTaxes ? item.cotrans || 0 : 0
    });
  ...
  const applyAutomaticRetention = () => {
    if (items.length === 0) {
      toast.error('Agregue al menos un detalle para aplicar la retención');
      return;
    }

    const eligibleItems = items.filter((item) => Number(item.subtotal || 0) > 0);

    if (eligibleItems.length === 0) {
      toast.error('No hay montos netos válidos para calcular la retención');
      return;
    }

    const retentionBase = Number(
      eligibleItems
    ...
    if (totalRetention <= 0) {
      toast.error('El monto de retención calculado debe ser mayor a cero');
      return;
    }
    ...
    const updatedItems = items.map((item) => {
      const itemBase = Number(item.subtotal || 0);

      if (itemBase <= 0) {
        return recalculateItemWithRetention(item, 0);
      }

      eligibleIndex += 1;

      const retentionAmount = eligibleIndex === eligibleItems.length
        ? Number((totalRetention - assignedRetention).toFixed(4))
        : Number(((totalRetention * itemBase) / retentionBase).toFixed(4));

      assignedRetention = Number((assignedRetention + retentionAmount).toFixed(4));

      return recalculateItemWithRetention(item, retentionAmount);
    });

    setItems(updatedItems);
    toast.success(`Retención 1% aplicada: ${formatMoney(totalRetention)}`);
  };

  const removeAutomaticRetention = () => {
    if (items.length === 0) {
      return;
    }

    setItems((prev) => prev.map((item) => recalculateItemWithRetention(item, 0)));
                ...
                <article key={item.localId} className="border rounded-xl p-4">
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                    <div>
                      ...
                      {Number(item.retention1 || 0) > 0 && (
                        <p className="text-sm text-red-700 mt-1">
                          Retención 1%: -{formatMoney(item.retention1)}
                        </p>
                      )}
                    </div>
          ...
          <div className="mt-4 space-y-3 text-sm">
            ...
            <div className="rounded-xl border border-red-100 bg-red-50 p-3 space-y-2">
              <p className="text-xs text-red-900">
                La retención se calcula automáticamente como el 1% del subtotal neto de los detalles y se resta del total con IVA incluido.
              </p>

              <div className="grid grid-cols-1 gap-2">
                <button
                  type="button"
                  onClick={applyAutomaticRetention}
                  disabled={items.length === 0}
                  className="inline-flex items-center justify-center rounded-lg bg-red-700 px-3 py-2 text-sm font-semibold text-white hover:bg-red-800 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  Aplicar retención 1%
                </button>

                {Number(invoiceTotals.retention1 || 0) > 0 && (
                  <button
                    type="button"
                    onClick={removeAutomaticRetention}
                    className="inline-flex items-center justify-center rounded-lg border border-red-200 bg-white px-3 py-2 text-sm font-semibold text-red-700 hover:bg-red-100"
                  >
                    Quitar retención
                  </button>
                )}
