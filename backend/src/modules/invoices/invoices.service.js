...
const RELATED_INVOICE_FIELDS_FOR_DOCUMENT_RELATED = [
  'id',
  'companyId',
  'documentTypeCode',
  'controlNumber',
...
const attachRelatedInvoiceForDocumentRelated = async (invoice, options = {}) => {
  if (!invoice?.relatedInvoiceId) return null;

  const relatedInvoice = await Invoice.findByPk(invoice.relatedInvoiceId, {
    attributes: RELATED_INVOICE_FIELDS_FOR_DOCUMENT_RELATED,
