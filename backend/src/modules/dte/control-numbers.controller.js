...
const listControlNumbers = async (req, res, next) => {
  try {
    const controlNumbers = await controlNumbersService.listControlNumbers(req.query);
...
const setLastControlSequence = async (req, res, next) => {
  try {
    const controlNumber = await controlNumbersService.setLastControlSequence(req.body);

    res.status(200).json({
      ok: true,
      message: 'Último correlativo actualizado correctamente',
      controlNumber
    });
  } catch (error) {
    next(error);
