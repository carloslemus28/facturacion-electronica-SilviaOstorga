...
const formatPersistedDateOnly = (value) => {
  ...
  /*
    Las fechas guardadas como DATETIME en MySQL ya representan el día fiscal
    del DTE. Para documentoRelacionado.fechaEmision no se debe volver a aplicar
    la zona horaria de la aplicación, porque una hora cercana a medianoche
    puede moverse al día anterior al reconstruir la Nota de Crédito.
  */
  return date.toISOString().slice(0, 10);
};

const getModelValue = (model, fieldName) => {
  if (!model || !fieldName) return null;

  if (Object.prototype.hasOwnProperty.call(model, fieldName)) {
    return model[fieldName];
  }

  if (model.dataValues && Object.prototype.hasOwnProperty.call(model.dataValues, fieldName)) {
    return model.dataValues[fieldName];
  }
...
const decodeSignedJwsPayload = (signedJws) => {
  const text = cleanString(signedJws);
...
const findOfficialDteJson = (candidate, depth = 0) => {
  ...
  if (typeof candidate === 'string') {
    ...
    const signedPayload = decodeSignedJwsPayload(trimmed);

    if (signedPayload) {
      return findOfficialDteJson(signedPayload, depth + 1);
    }

    const parsed = parseJsonValue(trimmed);

    if (parsed !== trimmed) {
      return findOfficialDteJson(parsed, depth + 1);
    }
  ...
  if (Array.isArray(candidate)) {
    for (const item of candidate) {
      const found = findOfficialDteJson(item, depth + 1);
  ...
  if (normalizeDateOnlyText(candidate.identificacion?.fecEmi)) {
    return candidate;
  }
  ...
  const nestedCandidates = [
    ...
    candidate.data,
    candidate.result,
    candidate.signerResponse
  ];

  for (const nestedCandidate of nestedCandidates) {
    const found = findOfficialDteJson(nestedCandidate, depth + 1);
...
const extractOfficialEmissionDateFromInvoice = (invoice) => {
  if (!invoice) return null;

  const mhResponseJson = parseJsonValue(getModelValue(invoice, 'mhResponseJson'));

  const candidates = [
    getModelValue(invoice, 'signedJws'),
    mhResponseJson?.signerResponse?.body,
    mhResponseJson?.payload?.documento,
    mhResponseJson?.haciendaPayload?.documento,
    mhResponseJson?.documento,
    mhResponseJson
  ];

  for (const candidate of candidates) {
    const officialDteJson = findOfficialDteJson(candidate);
    const emissionDate = normalizeDateOnlyText(officialDteJson?.identificacion?.fecEmi);

    if (emissionDate) return emissionDate;
  }

  return null;
};

const resolveInvoiceEmissionDateForDocumentRelated = (invoice) => {
  const officialEmissionDate = extractOfficialEmissionDateFromInvoice(invoice);

  if (officialEmissionDate) return officialEmissionDate;

  const issuedAt = getModelValue(invoice, 'issuedAt');

  return formatPersistedDateOnly(issuedAt) || formatDate(issuedAt);
};
...
const buildDocumentRelated = (invoice) => {
  ...
  const relatedIssuedAt = relatedInvoice
    ? getModelValue(relatedInvoice, 'issuedAt')
    : invoice.relatedInvoiceIssuedAt || invoice.issuedAt;

  const relatedEmissionDate = relatedInvoice
    ? resolveInvoiceEmissionDateForDocumentRelated(relatedInvoice)
    : null;
  ...
  return [
    {
      ...
      numeroDocumento,
      fechaEmision: relatedEmissionDate || formatPersistedDateOnly(relatedIssuedAt) || formatDate(relatedIssuedAt)
    }
  ];
...
module.exports = {
  ...
  resolveInvoiceEmissionDateForDocumentRelated,
